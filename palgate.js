...
class PalGateOpener {
  constructor(log, config, api) {
    ...
    this.informationService = new this.api.hap.Service.AccessoryInformation().setCharacteristic(this.api.hap.Characteristic.Manufacturer, "Pal Systems").setCharacteristic(this.api.hap.Characteristic.Model, "PalGate App");
    ...
    switch (this.accessoryType) {
      ...
      case "garagedoor":
        ...
        this.service.getCharacteristic(this.Characteristic.CurrentDoorState).on("get", this.handleCurrentDoorStateGet.bind(this));

        this.service.getCharacteristic(this.Characteristic.TargetDoorState).on("get", this.handleTargetDoorStateGet.bind(this)).on("set", this.handleTargetDoorStateSet.bind(this));
        break;
    }
  }
  ...
  handleTargetDoorStateSet(value, callback) {
    if (value == this.api.hap.Characteristic.TargetDoorState.OPEN) {
      ...
      xhr.setRequestHeader("x-bt-token", this.token);
      xhr.send();
